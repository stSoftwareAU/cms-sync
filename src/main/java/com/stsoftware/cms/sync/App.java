...
public class App extends AppCmdLine
{
    ...
    public void process() throws Exception {
        ...
        ThreadPool.schedule(() -> {
            ...
            while (true) {
                try {
                    JSONObject json1 = makeBuilder( "/ReST/v6/sync/SiteResource").addParameter("block", "1 min").addParameter("since", since).getResponseAndCheck().getContentAsJSON();
                    if (json1.has("since")) {
                        since = json1.getLong("since");
                    }
                    if (json1.getJSONArray("results").length() > 0) {
                        pullResources();
                    }
                }catch( Exception e)
                {
